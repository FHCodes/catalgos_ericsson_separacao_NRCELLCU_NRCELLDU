...
class DataProcessor:
    ...
    def __init__(self, path_name_xlsm, inventory_name):
    ...
    def process_data(self):
        try:
            ...
            msg_alerta_erro(
                'Ocorreu um erro ao gerar os jsons!',
    ...
    def _definir_informacoes_sheet_name(self):
        ...
        para construção do JSON de metadados.
        """
        sheet_info = {
            'data_sources': {
                ...
                'columns_of_interest': [
                    ...
                    'Delay',              # H
                    'Vendor',             # I
                    'Tecnologia/Grupo de Contadores',  # Q 
                    'Table Group'         # R
                ],
                'filter_col_index': 0
            },
            'data_sources_attr': {
                ...
                'columns_of_interest': [
                    ...
                    'Description',                      # I
                    'Example'                           # M — para defaultValue de constantes
                ],
                'filter_col_index': 0
            }
        }
    ...
    def _processar_data_sources(self, ds_filtered):
        ...
        for _, row in ds_filtered.iterrows():
            ...
            descricao = self._processar_descricao(row['col_4'])

            data_sources_list.append(
                (
                    row['col_0'],  # Inventory Name
                    row['col_1'],  # Table Name
                    row['col_2'],  # OssId e Id
                    row['col_3'],  # Schema
                    descricao,     # Description
                    row['col_5'],  # Period
                    row['col_6'],  # Delay
                    row['col_7'],  # Vendor
                    row['col_8'],  # Tecnologia/Grupo de Contadores
    ...
    def _processar_data_sources_attr(self, dsa_filtered):
        # Faz uma cópia segura para evitar SettingWithCopyWarning
